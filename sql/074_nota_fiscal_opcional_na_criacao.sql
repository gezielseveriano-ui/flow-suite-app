-- ============================================================================
-- FactoryView -- Expedição: nota fiscal deixa de ser obrigatória pra gerar o
-- romaneio. Na prática o romaneio (LE) é gerado e mandado pro setor fiscal
-- ANTES de existir número de NF -- exigir o número na hora de gerar travava
-- o fluxo real. NF continua podendo ser informada na hora (campo continua lá,
-- só não bloqueia mais vazio) ou preenchida depois pela Lista de Embarque
-- (editar_nota_fiscal_romaneio, já existente desde 064).
-- ============================================================================
...
begin
  ...
  values (p_pedido_id, auth.uid(), nullif(trim(p_nota_fiscal), ''))
